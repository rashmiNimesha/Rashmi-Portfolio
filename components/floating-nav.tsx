    ...
    <>
      ...
      >
        <div className="relative px-4 py-3 rounded-full bg-zinc-800/80 backdrop-blur-md border border-zinc-700/50 shadow-lg">
          <div className="absolute -inset-0.5 bg-gradient-to-r from-green-dark/20 to-green-medium/20 rounded-full blur opacity-50"></div>
          ...
          <div className="flex flex-col items-center justify-center h-full">
            {navItems.map((item) => (
              <Link
                key={item.name}
                href={item.href}
                className="px-8 py-4 text-2xl font-medium text-white hover:text-green-medium transition-colors"
                onClick={handleNavClick}
              >
                {item.name}
              </Link>
            ))}
            <Button className="mt-6 bg-gradient-to-r from-green-dark to-green-medium hover:from-green-medium hover:to-green-dark border-0">
              Resume
            </Button>
          </div>
        </motion.div>
